...
MODEL_NAME = 'hangul_recognition'
IMAGE_WIDTH = 64
IMAGE_HEIGHT = 64
EPOCH = 55000     #752 > 73세대
BATCH_SIZE = 100
...
def get_learning_data(files, num_labels):
    ...
    def _parse_function(proto):
        return tf.io.parse_single_example(proto, features=keys_to_features)
    
    features = file_queue.map(_parse_function)
    print(features)
...
#학습
def main(label_file, tfrecords_dir, output_dir, n_epoch):
    #라벨 읽어오기
    ...
    #학습률, 테스팅률, 시간 기록
    log_file = open("learning_log.txt", "w")
    ...
    file_writer = tf.train.SummaryWriter(root_logdir, sess.graph_def)
    """
    ...
    with tf.Session() as sess:
        ...
        #merge_hist = tf.summary.merge_all()
        #writer = tf.summary.FileWriter('./logs', sess.graph)
        ...
        for epoch in range(n_epoch):
            ...
            if epoch % 100 == 0:
                ...
                #summary = sess.run(merge_hist, feed_dict = {x:train_images, y_:train_labels})
                #writer.add_summary((summary, epoch))
                #writer.flush()
                plot_learn.append(float(train_accuracy))
            """
            ...
            plot_test.append(accuracy_percent)
            """
            ...
            if epoch % 100 == 0:
                ...
                log_file.write("Step %d, Training Accuracy %g, Training Loss %g" % (epoch, float(train_accuracy), loss_print))
...
if __name__ == '__main__':
    main(HANGUL_FILE, TFRECORDS_DIR, OUTPUT_DIR, EPOCH)
